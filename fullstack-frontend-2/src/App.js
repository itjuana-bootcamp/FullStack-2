...
function App() {
  ...
  const cartProductsQty = cartProducts.map(cp => cp.qty).reduce((prev, curr) => prev + curr, 0)
  ...
  const onAdd = id => {
    ...
    if (currentProductIndex >= 0)
      ...
      tempCartProducts.push({...product, qty: 1})
  ...
  return (
    <Fragment>
      ...
      <Routes>
        ...
        />
        <Route path="/" element={<Shop products={allProducts} onAdd={onAdd}/>} />
        <Route
          path="/cart"
          element={
            <CartPage
